...
/* Atribui ao evento keypress do input data de nascimento a função para formatar o data (dd/mm/yyyy) */
var inputDataNascimento = document.getElementById("data_entrega");
if (inputDataNascimento != null && inputDataNascimento.addEventListener) {                   
    inputDataNascimento.addEventListener("keypress", function(){mascaraTexto(this, '##/##/####')});
} else if (inputDataNascimento != null && inputDataNascimento.attachEvent) {                  
    inputDataNascimento.attachEvent("onkeypress", function(){mascaraTexto(this, '##/##/####')});
...
function validaCadastro(evt){
	...
	caixa_projeto = document.querySelector('.msg-projeto');
	if(nome.value == ""){
		caixa_projeto.innerHTML = "Favor preencher o projeto";
		caixa_projeto.style.display = 'block';
		contErro += 1;
	}else{
		caixa_projeto.style.display = 'none';
	}
	...
	/* Validação do campo Data de Entrega */
	caixa_data_entrega = document.querySelector('.msg-data');
	if(caixa_data_entrega.value == ""){
		caixa_data_entrega.innerHTML = "Favor preencher a Data de Entrega";
		caixa_data_entrega.style.display = 'block';
		contErro += 1;
	}else{
		caixa_data_entrega.style.display = 'none';
	}
